...
void SVGParser::parseTexture( XMLElement* xml ) {
  ...
  int err = lodepng::decode(pixels, width, height, file);
  if (err) {
...
void SVGParser::parseColorTri( XMLElement* xml, ColorTri* ctri ) {
  ...
  float x, y;
  points >> x >> y; ctri->a = Vector2D(x,y);
  points >> x >> y; ctri->b = Vector2D(x,y);
  points >> x >> y; ctri->c = Vector2D(x,y);

  stringstream colors (xml->Attribute( "colors" ));

  float r,g,b,a;
  colors >> r >> g >> b >> a; ctri->ac = Color(r,g,b,a);
  colors >> r >> g >> b >> a; ctri->bc = Color(r,g,b,a);
  colors >> r >> g >> b >> a; ctri->cc = Color(r,g,b,a);
...
void SVGParser::parseTexTri( XMLElement* xml, TexTri* ttri ) {
  ...
  float x, y;
  points >> x >> y; ttri->a = Vector2D(x,y);
  points >> x >> y; ttri->b = Vector2D(x,y);
  points >> x >> y; ttri->c = Vector2D(x,y);

  stringstream uvs (xml->Attribute( "uvs" ));

  uvs >> x >> y; ttri->a_uv = Vector2D(x,y);
  uvs >> x >> y; ttri->b_uv = Vector2D(x,y);
  uvs >> x >> y; ttri->c_uv = Vector2D(x,y);
